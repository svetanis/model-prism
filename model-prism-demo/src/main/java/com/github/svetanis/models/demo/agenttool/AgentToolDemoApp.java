...
/**
 * Demonstrates {@link GoogleSearchAgentTool} - an
 * {@link com.google.adk.tools.AgentTool} that wraps a Gemini-powered search
 * sub-agent and exposes it as a callable tool to an outer agent running on a
 * completely different (non-Google) model.
 *
 * <p>
 * The inner Gemini agent is invoked transparently via ADK's tool-dispatch
 * mechanism - the outer Groq agent simply sees a function called
 * {@code google_search_agent} in its tool declarations.
 */
